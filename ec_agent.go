...
type Control_Json struct {
    Title       string `json:"title"`
    Command     string `json:"command"`
    Baseline    string `json:"baseline"`
}

func (c Control_Json) GetCommand() string{
    return c.Command
}

func (c Control_Json) GetTitle() string{
    return c.Title
}

func (c Control_Json) GetBaseline() string{
    return c.Baseline
}

func (p Control_Json) toString() string {
    return toJson(p)
}
...
func getControlJson(manifest string) []Control_Json {
...
func main() {
    ...
    for _, controlJ:= range baseline {
        var b bytes.Buffer

        data := controlJ.GetCommand()
        ...
        co := Control{title: controlJ.GetTitle(),
                        command: controlJ.GetCommand(),
                        output: s,
                        dateExe: DateTimeNow(),
                        baseline: controlJ.GetBaseline()}

        controlO = append(controlO, co)

        fmt.Println("- Done executing","[", controlJ.GetTitle(), "]")
